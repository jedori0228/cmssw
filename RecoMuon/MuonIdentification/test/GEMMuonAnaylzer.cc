...

{
  ...
  for(unsigned int i=0; i<gensize; ++i) {
    ...
    if ( (CurrentParticle.status()==1) && ( (CurrentParticle.pdgId()==13)  || (CurrentParticle.pdgId()==-13) ) ){  
      ...
      } // END of generalTracks loop

      // NO matched ME0Muon to gen muon
      if (MatchedID == -1){
      ...
      } // END (current particle matched to ME0Muon)
    ...
    } // END prompt muon selection
  } // END for(unsigned int i=0; i<gensize; ++i)
 ...
 //Track Association by Chi2 or hits:
  ...
  int i_me0muon=0;
  for (std::vector<ME0Muon>::const_iterator thisMuon = OurMuons->begin(); thisMuon != OurMuons->end(); ++thisMuon, ++i_me0muon){
    if (!muon::isGoodMuon(*thisMuon, muon::Tight)) continue;
